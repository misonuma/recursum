...
# +
PROXIES = {
    'http': 'http://10.7.30.1:8080',
    'https': 'https://10.7.30.1:8080',
}
...
flags.DEFINE_string("task_desc", None, "The description of the task to train.")
...
def main(_):
  ...
  processors = {
      ...
      "arg": classifier_utils.ArgProcessor,
      "spec": classifier_utils.SpecProcessor,
  }
  ...
  task_desc = FLAGS.task_desc.lower()

  if task_name not in processors:
    raise ValueError("Task not found: %s" % (task_name))

  if FLAGS.task_desc is None:
    processor = processors[task_name](
          use_spm=True if FLAGS.spm_model_file else False,
          do_lower_case=FLAGS.do_lower_case)
  else:
    processor = processors[task_desc](
          use_spm=True if FLAGS.spm_model_file else False,
          do_lower_case=FLAGS.do_lower_case)
...
#           best_metric_global_step = int(best_metric_global_step)
          best_metric_global_step = int(global_step)
